    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < 30; i++) {
            ...
            try {
                ...
                System.out.println(new Date() + " Send mq order message success! Topic is:" + MqConfig.ORDER_TOPIC + " msgId is: " + sendResult.getMessageId());
            } catch (ONSClientException e) {
                // 消息发送失败，需要进行重试处理，可重新发送这条消息或持久化这条数据进行补偿处理
                System.out.println(new Date() + " Send mq order message failed! Topic is:" + MqConfig.TOPIC);
                e.printStackTrace();
            }
        }
    }
